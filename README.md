...
预热 Claude desktop 的 SSH remote agent + claude CLI binary。desktop 通过 SSH 远程到这台机器时需要 server binary（5.9MB）+ CLI（213MB 解压后）；直连 `downloads.claude.ai` 不稳时会卡在 "Configuring machine..."。

LaunchAgent 监听 `app.asar`（desktop 升级会替换它），一变化就重新预热：读 ASAR 内嵌 manifest 拿 checksum → 走系统代理下载 zst → sha256 校验 → 原子解压落盘。文件已存在且 checksum 对则直接 skip，零成本。

依赖：`brew install zstd`
...
日志：`/tmp/claude-ssh-prep.{out,err}.log`。两台 mac 都装构成对称 mesh，互不依赖。
